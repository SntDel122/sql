...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 07-03-2024 a las 15:59:55
...
CREATE TABLE `cliente` (
  ...
  `tipo_documento` enum('c','ce','nit','cut') NOT NULL,
  ...
  `estado_civil` enum('soltero','casado','divorciado','union_libre','viudo') NOT NULL,
  `tipo_cliente` enum('detallista','mayorista','empresarial') NOT NULL,
...
CREATE TABLE `empleado` (
  ...
  `nivel_estudios` enum('primaria','bachilletaro','tecnico','tecnologo','profesional','otro') DEFAULT NULL,
  `eps` enum('sanitas','sura','capital salud','nueva eps','compensar',' famisanar') DEFAULT NULL,
...
--
ALTER TABLE `cliente`
  ADD CONSTRAINT `cliente_ibfk_1` FOREIGN KEY (`empleado_cod`) REFERENCES `empleado` (`cod_empleado`) ON DELETE CASCADE ON UPDATE CASCADE;
...
--
ALTER TABLE `productos`
  ADD CONSTRAINT `productos_ibfk_1` FOREIGN KEY (`categor_cod`) REFERENCES `categoria` (`cod_categoria`) ON DELETE CASCADE ON UPDATE CASCADE,
